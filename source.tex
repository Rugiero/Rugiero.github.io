...
\def\googlescholar{https://scholar.google.com/citations?user=21n13EQAAAAJ&hl=en/}
%%\def\cv{https://www.dropbox.com/s/jem3l3jabcyex9s/Curriculum_Vitae_Ilari_Angervuori.pdf?dl=0}
\def\cv{Ilari_Angervuori_CV.pdf}
...
\def\reviewcert2{certificate2.pdf}
\makeindex
...
\author
    ...
    \date
        ...
        \begin{center}
          ...
          \htmladdnormallink{Google Scholar}{\googlescholar}\\
...
%          \htmladdnormallink{Academic CV}{\academiccv}\\
        ...
        %% \section{About me}
         ...
         I am a Finnish researcher and engineer who loves mathematics.
        ...
        I keep a blog that you can find in the blog posts section. It covers all kinds of subjects I encounter in my daily professional life. Stuff that I have found interesting. 
        ...
        \begin{itemize}
        \item Expected 2026, PhD, \htmladdnormallink{``Narrow-Beam Low Earth Orbit Communications and Stochastic Geometry: Non-Temporal and Temporal Interference Analysis,''}{dissertation.pdf} Department of Signal Processing, Aalto University\\
...
%%         \item Ilari Angervuori, Wichman Risto, Niloofar Okati, Taneli Riihonen, ``On routing protocols in inter-satellite communications,'' Proceedings of XXXV Finnish URSI Convention on Radio Science, 2019
        ...
        \begin{itemize}
        \item PhD dissertation (draft), ``Narrow-Beam LEO Communications and Stochastic Geometry: Non-Temporal and Temporal Analysis,'' \htmladdnormallink{preprint}{dissertation.pdf}, Department of Electrical Engineering, Aalto University, 2025          
        ...
        \begin{itemize}
        \item 2024, IEEE Transactions, Mobile Computing, \htmladdnormallink{2024 Review Certificate}{}
        \item 2025, IEEE Transactions, Mobile Computing, \htmladdnormallink{2025 Review Certificate}{}
        \end{itemize}        
        ...
        The clear trend is that \htmladdnormallink{artificial intelligence}{https://en.wikipedia.org/wiki/Artificial_intelligence} (AI) and \htmladdnormallink{machine learning}{https://en.wikipedia.org/wiki/Machine_learning} (ML) are current hot topics, particularly in wireless networks and signal processing. Another active research topic is \htmladdnormallink{stochastic geometry}{https://en.wikipedia.org/wiki/Stochastic_geometry_models_of_wireless_networks} (SG). In this regard, some perceive SG and ML as competitors, suggesting ML has surpassed SG in practical impact. However, this comparison is a poor perspective and not the only way to look at it. The ongoing SG research remains valuable, beyond being an academic exercise. SG can and will be utilised in modern ML by providing models and, further, \textbf{hypothesis classes} of models for ML algorithms. In this post, we will introduce some of the most recent advancements in SG modelling of satellite communication systems and provide a concrete example of its application in ML.

        \begin{quotation}
          \textit{For instance, SG models can be integrated as a hypothesis class in the learning process of ML.}
                           \begin{flushright}
                             --- Yassine H Mamouche et al., [1].
                           \end{flushright}
                         \end{quotation}

        \begin{figure}
          \includegraphics[width=\linewidth]{SGandML.png}
        ...
        Let us formulate the working definitions of the ML and SG. We focus on the signal processing of wireless networks, particularly interference modelling.
        \begin{itemize}
        \item{
          SG encompasses \textbf{stochastic models} of wireless networks, allowing the investigation of many statistical properties, such as interference power distribution and coverage. These expressions are usually presented in \textbf{mathematical form}; tractable, closed-form formulas are the most insightful and useful.} 
        \item{ML encompasses learning algorithms that ultimately utilise empirical \textbf{raw data}, and they are powerful tools for signal interpolation and prediction. The results are usually numerical.}
        \end{itemize}

        
                 The landscape of ML algorithms is vast; however, they are often based on \htmladdnormallink{Gaussian process regression}{https://en.wikipedia.org/wiki/Kriging} (GPR). GPR is a signal prediction method that relies on estimating the signal from a set of samples (or observations) and a \htmladdnormallink{correlation}{https://en.wikipedia.org/wiki/Correlation} function. More specifically, an \htmladdnormallink{autocovariance}{https://en.wikipedia.org/wiki/Autocorrelation} function (also referred to as a kernel) is at the heart of GPR. The autocovariance function can be arbitrary, and it should be carefully chosen to get meaningful results. One way is to numerically estimate the autocovariance from \htmladdnormallink{training data}{https://en.wikipedia.org/wiki/Training,_validation,_and_test_data_sets}. Whether the training data is empirical data from an actual system or generated synthetically using simulation tools, numerical statistical estimates lack generality and physical insight and have a concomitant dependence on local, empirical data and circumstances. How to interpret such data if we don't have understand what qualities we are looking for? It is time for the SG to step into the arena: A comprehensive, tractable, and solid wireless network SG system model provides ML models that guide us effectively in inferring patterns from empirical data.
                 

                 We will demonstrate an example of estimating the power of a non-stationary Gaussian interference signal at a \htmladdnormallink{low Earth orbit}{https://en.wikipedia.org/wiki/Low_Earth_orbit} (LEO) satellite receiver. The estimation is based on the GPR and a theoretical SG model of the interference power distribution and its autocovariance function. The results are compared with a \htmladdnormallink{moving average}{https://en.wikipedia.org/wiki/Moving_average} (MA). 
                 
                 
                 The theoretical settings are 
                 \begin{enumerate}
                 \item{The interferers (natural sources or transmitters) of bandwidth $1$ kHz are distributed according to the \htmladdnormallink{Poisson point process}{https://en.wikipedia.org/wiki/Poisson_point_process} (PPP) on the Earth surface with a density $\lambda=7 \times 10^{-3}/$km$^2$, causing interference at the LEO satellite. Each interferer transmits at power $p_t=1$.} 
                 \item{The LEO satellite has a \textit{narrow} antenna pattern with a half-power ($-3$ dB) beamwidth $\varphi_{\text{RX}}=1.6^{\circ}=0.027925$ rad, steered toward the Earth's center. This means the beam center elevation angle w.r.t the satellite is $\epsilon = 90^{\circ}=\pi/2$ rad.}
                 \item{The LEO satellite is at $h=200$ km moving at its orbital speed $\left|\vec{v}_{\text{sat}}\right|=v_{\text{sat}}=7.4$ km/s.}
                \item{We model Rician fast \htmladdnormallink{fading}{https://en.wikipedia.org/wiki/Fading}. No shadowing, cloud attenuation, or similar are modelled. 
                 }
                 \end{enumerate}
                 As the satellite moves, the magnitude of the aggregate interference from the randomly distributed interferers varies. A rigorous analysis of the interference statistics is presented in my thesis \htmladdnormallink{[2]}{dissertation.pdf} and in article \htmladdnormallink{[4]}{https://arxiv.org/pdf/2607.11602}, where the results are validated against a realistic Monte Carlo simulation model.

                 The key insights acquired from the SG analysis are as follows. 

                 
                 \begin{figure}                           
                   \includegraphics[width=\linewidth]{SGandMLsysmod.png}
                   \caption{A sketch of the system model.}
                 \end{figure}

                 \begin{enumerate}
                 \item[5.]{Under fairly general settings (for example, assuming a Gaussian waveform for each interferer), the interference waveform at the LEO satellite can be modelled as
                   $$I=I(t)=\sqrt{P(t)}X(t), $$
                   where $X=X(t)$ is a zero-mean Gaussian waveform with $\mathbb{E}(|X|^2)=1$, which is modulated by the power $P=P(t)$ (see \htmladdnormallink{[2]}{dissertation.pdf}, Section 3.4).}
                 \item[6.]{The autocovariance
                   $K_P(\tau) \triangleq \mathbb{E}\left[(P(t)-\mathbb{E}(P(t)))(P(\tau-t)-\mathbb{E}(P(\tau-t)))\right]$
                   of $P$ at lag $\tau$ has the \htmladdnormallink{Gaussian form}{https://en.wikipedia.org/wiki/Radial_basis_function_kernel}
                   \begin{align*}
                     K_{P}(\tau) &= \frac{1}{2} \underbrace{\frac{\pi \lambda p_t^2}{\log(2)}  \left(\frac{h \varphi_{\text{RX}}}{\sin^2(\epsilon)}\right)^2}_{ k\triangleq} \left( E(H)^2\exp\left\{-\tau^2\underbrace{\frac{v_{\text{sat}}^2\sin^4(\epsilon)}{2 h^2 \varphi_{\text{RX}}^2}}_{D\triangleq } \log(2)\right\} \tag{2} + K_H(\tau) \right)\\
                     &=\frac{k}{2} \left(E(H)^2\exp\left\{-D\tau^2 \log(2)/2\right\} +   \text{var}(H) \text{triang}(\tau/\tau_c) \right),
                   \end{align*}
                   where $H \geq 0$ is an i.i.d. deterministic or random fading gain of the typical transmitter. The Gaussian left-hand-side term corresponds to satellite movement and antenna attenuation of the interferers, and the right-hand-side term corresponds to the fading component. The autocovariance function is modelled using a triangular function with coherence time $\tau_c$ (samples separated by a time interval greater than $\tau_c$ are uncorrelated). The parameters $k,D>0$ are constants that depend on $\varphi_{\text{RX}}$, $p_t$, $\epsilon$,  $h$, $v_{\text{sat}}$, and $\lambda$ (analogous derivation for the amplitude in \htmladdnormallink{[2]}{dissertation.pdf}, Section 4.2, and for the Pearson correlation coefficient \htmladdnormallink{[4]}{https://arxiv.org/pdf/2607.11602}, Eq. (21)).  For our system model settings 1-4, $k\approx 0.5$ and $D\approx 1.7555$.
                   
 If left unfixed, $k$, $D$, $\mathbb{E}(P)$, $\mathbb{E}(H)$, $\text{var}(H)$, and $\tau_c$ are \htmladdnormallink{hyperparameters}{https://en.wikipedia.org/wiki/Hyperparameter_(machine_learning)} learned from the data in the machine learning process. Generally, they can be dependent on time; however, we treat them as constants. In the correlation model, too large or too small $D$ or $1/\tau_c$ yields \htmladdnormallink{overfitting}{https://en.wikipedia.org/wiki/Overfitting} and underfitting, respectively. 
                 }
                   
                 \item[7.]{The normalised interference power is approximately \htmladdnormallink{gamma distributed}{https://en.wikipedia.org/wiki/Gamma_distribution} with the mean
                   \begin{equation}
                     \mathbb{E}(P)=\mathbb{E}(I^2)=\text{var}(I)= k \mathbb{E}(H)
                   \end{equation}
                   and the variance
                   \begin{equation}
                     \text{var}(P)=k\mathbb{E}(H^2)/2.
                   \end{equation}
                   (See \htmladdnormallink{[4]}{https://arxiv.org/pdf/2607.11602}, Eqs. (18), (19) and (29)).}
                 ...
                 The correlation model 6, combined with the gamma distribution model 7, defines the hypothesis class in which we search for the power $P$. This process is computationally highly efficient and will serve as a prior model for training the ML algorithm.
...
In Figures 1 (a) and (b), we estimate $P(t)$ from a sampled ($0.1$ kHz) realisation of the interference waveform $I(t)$ by using GPR and non-\htmladdnormallink{causal}{https://en.wikipedia.org/wiki/Causal_filter} MA. The ``true'' signal is generated following the distribution properties 5-7 (i.e., is not generated by simulating the PPP). Furthermore, for simplicity, in the ML algorithm, the parameters are fixed at the values $k\approx 0.5$, $D\approx 1.7555$, and $\mathbb{E}(P)=1$, and are not treated as hyperparameters. We use the samples from $|I(t)|^2$ to estimate the interference power $P(t)$. Each sample $z_i$ is interpreted as a noisy measurement of $P(t)$: $z_i= P(t_i) + \mathcal{N}(0,\sigma^2)$, where $\sigma^2=3$ is set to the expected variance of $|I(t)|^2$. The MA window size is empirically optimised, and apart from the slight overfitting, the MA captures $P(t)$ reasonably well. The GPR prediction clearly captures the smoothness of $P(t)$ better. It captures $P(t)$ from $t=0$ onward, which is impossible for the non-causal, centralised MA, which starts at $t\approx 0.5$ s. The \htmladdnormallink{forecast}{https://en.wikipedia.org/wiki/Predictive_analytics} region in the figure corresponds to a causal estimation of the future signal based on the past values. Both predictions improve as the sampling frequency is increased.


This was a simple hands-on demonstration of how SG is utilised in ML satellite communication models.
...
# === Parameters ===
T_total = 10.0       # simulate P(t), I(t)
T_obs = 7.0         # "observation" time 
dt = 0.001           # fine time step for continuous path 
t = np.arange(0, T_total, dt)
N = len(t)

fs_sample = 100.0     # sampling of |I|^2 
sample_interval = 1.0 / fs_sample  
...
# === 1. Construct latent Gaussian process for P(t) with covariance C(tau) = k * exp(-D tau^2 log(2) / 2) ===
tau_full = t[:, None] - t[None, :]
k_emp = 0.5
D_emp = 1.7555
C_full = k_emp * np.exp(-D_emp * tau_full**2 * np.log(2) / 2)
...
# # === 4. 100-sample moving average on sampled powers (0..T_obs) ===
window_size = 100
...
# Var(|I|^2 | P=p) = 2 p^2, E[P^2]=1.5 => average Var \approx 3
noise_var = 3.0
...
# === 5. GPR prediction of P(t) on from noisy power samples ===
# GP prior: P ~ GP(mean=mean_P, cov(t,s) = 0.5*exp(-3(t-s)^2))
# Observations: z_i = |I(t_i)|^2 \approx P(t_i) + N(0, noise_var)
...
k_prior = 0.5
D_prior = 1.7555
C_ss = k_prior * np.exp(-D_prior * tau_ss**2 * np.log(2) / 2)
# Covariance between all times (0..T_total) and training points
tau_fs = t[:, None] - t_train[None, :]
C_fs = k_prior * np.exp(-D_prior * tau_fs**2 * np.log(2) / 2)
...
# GPR mean and ±2 \sigma 
ax1.plot(t, P_gp_mean, label='GPR prediction of P(t)', color='C1', linewidth=2)
# 90 percent confidence interval
ax1.fill_between(t, P_gp_mean - 1.645*P_gp_std, P_gp_mean + 1.645*P_gp_std,
                 color='C1', alpha=0.2, label='GPR 90% CI')

# 100-sample MA on [0, T_obs]
ax1.plot(t_ma, power_ma, 'r-', label='MA of |I|² (0–7 s)')
...
ax1.set_title('Fig 1(a): True P(t), GPR estimate & forecast, and 100-sample MA & forecast', fontsize=12)
ax1.grid(True)
ax1.legend(fontsize=12)

# ---------- Subplot (b): I(t) and samples ----------
ax2.plot(t, I, label='I(t) = X(t)\sqrt{P(t)}', color='tab:orange')
ax2.plot(t_sampled, I_sampled, 'ko', label='Samples of I(t) (0–7 s, 0.1 kHz)', markersize=1)
...
References:
\begin{itemize}
\item[[1]] Y. Hmamouche, M. Benjillali, S. Saoudi, H. Yanikomeroglu and M. D. Renzo, "New Trends in Stochastic Geometry for Wireless Networks: A Tutorial and Survey," in Proceedings of the IEEE, vol. 109, no. 7, pp. 1200-1252, July 2021, doi: 10.1109/JPROC.2021.3061778 
\item[[2]] \htmladdnormallink{Ilari Angervuori, Narrow-Beam Low Earth Orbit Communications and Stochastic Geometry: Non-Temporal and Temporal Interference Analysis, Thesis Draft; Aalto University}{dissertation.pdf}
\item[[3]] \htmladdnormallink{Maiju Oksanen, Stokastinen geometria langattoman
  ...
  \item[[4]] \htmladdnormallink{I. Angervuori, A. Afridi, R. Wichman, Spatial and Temporal Correlation of Interference in a Narrow Multibeam LEO Satellite Random Access Network, arXiv:2607.11602}{https://arxiv.org/abs/2607.11602}
...
\subsection{May---Signal fading at a satellite receiver and the coherence time}
\begin{figure}
...
It is also interesting to compare the audiolised noise signals:
...
\subsection{May---Rayleigh fading audiolised}
...
References: 
\begin{itemize}
\item \htmladdnormallink{Scattering Problem: Matlab PDE Modeller App}{https://se.mathworks.com/help/pde/ug/scattering-problem.html}
...
gpg --generate-key
\end{verbatim}
\item Copy the name of the key and initialise pass.\\
\begin{verbatim}
...
\item Now, we can initialise the Git repository with these keys. Move the public key and private key through a safe channel to a computer you wish to use Pass in. Import the keys to the machine: \\
\begin{verbatim}
...
\end{itemize}
    ...
    \subsection{Fast Fourier transform in GNU Octave}
    ...
    Octave calculates the FFT of a discrete signal.  In the following code, we calculate the normalised FFT and plot the frequency spectrum w.r.t. the frequency. Signal vector and sampling frequency are given as input.
...
\end{align}
In $(a)$, we used the asymptotic behaviour of the gamma distribution
$$
...
We used deterministic signals, but the same remarks apply to random signals. For example, for uniformly random phases $\phi_1,\phi_2 \in [0,2 \pi]$, let $S_1(t) = \cos(2 \pi t + \phi_1)$ and $S_2(t) = \cos(2 \pi t + \phi_2)$;
...
\subsection{A representation of the generalised hypergeometric function $_3F_2$}
