...
\end{align}
but the result greatly differs from the derivation in $(2)$! Some of the readers may be familiar with the   \htmladdnormallink{Cauchy–Schwarz inequality}{https://en.wikipedia.org/wiki/Cauchy-Schwarz_inequality#Probability_theory}, and already know that the left-hand side of the equation (2) does not generally equal the lhs of (3). Still, e.g. in point process models of wireless networks, the signal powers are often summed as in (3). The often unmentioned assumption behind this has everything to do with the independence of the signals:

let us assume that $S_1$ and $S_2$ are some random variables (or deterministic variables, as a deterministic variable just is a random variable with a degenerate distribution). We have that
\begin{equation}
...
For example, the uncorrelation assumption holds for Gaussian noises. Furthermore, we can often consider that all distinct signals in the same communication system are uncorrelated. Even if the signals would be slightly correlated, the Cauchy-Schwarz inequality guarantees that the sum of the powers is an upper bound.
