...
\item \htmladdnormallink{Interface conditions for electromagnetic fields (Wikipedia article)}{https://en.wikipedia.org/wiki/Interface_conditions_for_electromagnetic_fields}
  ...
  It is a well known fact that some point processes converges to Poisson point process after sufficient amount of right transformations. This is of course why the Poisson p.p appears everywhere in the nature; for example, grain of sands in a pile of grains – made by a child at a beach on a sunny day – will eventually spread by wind (or water) around the beach. The grains will be randomly displaced here and there forming something approximating a realization of a Poisson p.p., even though the original pile was more ordered.

  Let's play the game with a pile of sand spreading to a flat area by random translations of the grains. 

  Consider a pile of sand at orgio. Let's assume that the pile is small enough so that it is concerned in a single point, i.e. the origo. Lets make an natural assumption that the amount of grains in the pile is  \htmladdnormallink{Poisson distributed}{https://en.wikipedia.org/wiki/Poisson_distribution}. 

  Suppose that after a while, wind moves the individual sand grain from the pile directing them with uniformly distributed angle and to a random distance between $(-\infty, \infty)$. In other words, the probability that a sand grain is moved from the origo to a location $Y_j$ inside a circle $B(r_1,r_2)$ is $2\int_{r_1}^{r^2} \frac{1}{\sqrt{2 \pi}}e^{-y^2} dy.$ 
  ...
  Evaluating the  \htmladdnormallink{Poisson Laplace functional}{https://en.wikipedia.org/wiki/Poisson_point_process#Laplace_functionals} $\mathcal{L}(g)$ of the sand pile with $ g = - \log \left(   \int_{0}^{\infty}  \exp \left[  -f(y)  \right] \frac{e^{-y^2}}{\sqrt{2 \pi}} dy \right) \right] $ we get;
    \begin{align*}
      \mathcal{L}(g) = \mathcal{L}_{\tau}(f)& = \exp \left[  \left{-\int_{\mathbb{R}^2} 1 -  e^{\log \left(   \int_{0}^{\infty}  \exp \left[  -f(y)   \right] \frac{2e^{-y^2}}{\sqrt{2 \pi}} dy \right) }\Lambda(dx) \right]    \\
    ...
    \end{align*}
where $N$ denotes the expected number of points in the sand pile (\htmladdnormallink{Campbell's theorem}{https://en.wikipedia.org/wiki/Campbell\%27s_theorem_(probability)}). But the formula $(1)$ is the Laplace functional  of the Poisson p.p. on $[0, \infty)$ with a density parameter $\sqrt{\frac{2}{\pi}}e^{-x^2} N.$ As the direction of the translation of a sand grain was uniformly distributed in $[0, 2\pi]$, we can conclude that the sand grains are Poisson distributed according to the Poisson p.p. with the  two-dimensional multivariate Gaussian distribution as the density.

  In fact, the result above applies to any translations as long as the translation is independently applied to every point (or grain); Poisson point process remains Poisson in a transformation. Yes, we had to make the Poisson assumption also for the pile, so that the result above could be derived. But indeed, we can show that certain processes approach Poisson p.p. when the points are translated. However, I leave this is out of scope of this blog entry for now.

  \begin{figure}
    \includegraphics[width=\linewidth]{spread.pdf}
    \caption{Wavelengths bigger than the diameter of the tunnel will not get through}
\end{figure}.
