...
\bibitem[1]{pass}\htmladdnormallink{Password Store}{\pass}
  ...
  Interference in big crowds gets so large that data transmission rates will lower to the level of jamming. This can be seen for example in crowded festival areas, when calling to a friend is impossible.

  \begin{figure}
  \includegraphics[width=\linewidth]{rician.gif}
  \caption{Interference field developing in time. Complex surrounding is modeled as random multi path signal propagation from random locations moving to random directions in time. Also, a line-of-sight component is assumed. Red occurrences will cause remarkable disturbance in communication.}
\end{figure}


  References:

\bibitem{hurr}François Baccelli, Bartlomiej Blaszczyszyn \htmladdnormallink{Stochastic Geometry and Wireless Networks, Volume I -Theory}{\hal}
