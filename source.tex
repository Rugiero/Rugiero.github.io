...
\def\reviewcert_2{certificate2.pdf}
\makeindex
...
%%         \item Ilari Angervuori, Wichman Risto, Niloofar Okati, Taneli Riihonen, ``On routing protocols in inter-satellite communications,'' Proceedings of XXXV Finnish URSI Convention on Radio Science, 2019
        ...
        \item 2025, IEEE Transactions, Mobile Computing, \htmladdnormallink{2025 Review Certificate}{\reviewcert_2}
        \end{itemize}        
        ...
        \end{figure}
                 ...
                 Let me formulate the working definitions of the ML and SG. We focus on the signal processing of wireless networks, particularly interference modelling.
                 \begin{itemize}
                 ...
                 \item{The LEO satellite has an antenna pattern with half $-3$ dB beamwidth $\varphi_{\text{RX}}=1.6^{\circ}=0.027925$ rad steered towards the Earth center. This means the beam center elevation angle w.r.t. the satellite is $\epsilon = 90^{\circ}=\pi/2$ rad.}
                 \item{The LEO satellite is at $h=200$ km moving at its orbital speed $\left|\vec{v}_{\text{sat}}\right|=v_{\text{sat}}=7.4$ km/s.}
                \item{While feasible, for simplicity, we do not model fast \htmladdnormallink{fading}{https://en.wikipedia.org/wiki/Fading}, shadowing, or other signal attenuation. For the narrow beam, the spatial path losses and Doppler shifts are approximately identical for all (relevant) interferers.
                 }
                 \end{enumerate}
                 As the satellite moves, the magnitude of the aggregate interference from the randomly distributed interferers varies. A rigorous analysis of the interference statistics is presented in my thesis \htmladdnormallink{[2]}{dissertation.pdf} and in article \htmladdnormallink{[4]}{https://arxiv.org/pdf/2607.11602}, where we also validate the results against a realistic Monte Carlo simulation model. The key insights acquired from the SG analysis are as follows. 
                 ...
                 \item[6.]{The normalised interference power (\textit{i.e.}, variance of $I$) is approximately \htmladdnormallink{gamma distributed}{https://en.wikipedia.org/wiki/Gamma_distribution} with the mean $\mathbb{E}(P)=\mathbb{E}(I^2)=\text{var}(I)=1$ and the variance $\text{var}(P)=1/2$ (see \htmladdnormallink{[4]}{https://arxiv.org/pdf/2607.11602}, Eqs. (18), (19) and (29)).}
                 \item[7.]{The autocovariance of $P(t)$ at lag $\tau$ has the \htmladdnormallink{Gaussian form}{https://en.wikipedia.org/wiki/Radial_basis_function_kernel}
                   \begin{align*}
                     K_{P}(\tau)&=k \exp\left\{-D\tau^2 \log(2)/2\right\}\\
                     &= \underbrace{\frac{p_t^2}{2\log(2)}  \pi \lambda\left(\frac{h \varphi_{\text{RX}}}{\sin^2(\epsilon)}\right)^2}_{ k\triangleq} \exp\left\{-\tau^2\underbrace{v_{\text{sat}}^2\sin^4(\epsilon)/(h^2 \varphi_{\text{RX}}^2)}_{D\triangleq } \log(2)/2\right\}, \tag{2}
                     \end{align*}
                   where $k,D>0$ are constant parameters that depend on $\varphi_{\text{RX}}$, $p_t$, $\epsilon$,  $h$, $v_{\text{sat}}$ and $\lambda$ (see \htmladdnormallink{[4]}{https://arxiv.org/pdf/2607.11602}, Eq. (21)).

                   For our system model settings 1-4, $k\approx 0.5$ and $D\approx 1.7555$. }
                 \end{enumerate}
                 
                 If left unfixed, $k=\text{var}(P)$, $D$, and $\mathbb{E}(P)$ are \htmladdnormallink{hyperparameters}{https://en.wikipedia.org/wiki/Hyperparameter_(machine_learning)} learned from the data in the machine learning process. Then the gamma distribution model 6, combined with the correlation model 7, forms the hypothesis class within which we search for the power of $I$. In the correlation model, too large or too small $D$ yields \htmladdnormallink{overfitting}{https://en.wikipedia.org/wiki/Overfitting} and underfitting, respectively. 
...
# === 1. Construct latent Gaussian process for P(t) with covariance C(τ) = k * exp(-D τ^2 log(2) / 2) ===
