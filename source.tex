...
\author
    ...
    \date
        ...
        \pagebreak
        \section{About me}
        I am a Finnish mathematician interested in engineering and particularly wireless communications. Please jump to my professional records in the subsections below.
        ...
        \htmladdnormallink{Poisson point process}{https://en.wikipedia.org/wiki/Poisson_point_process} can be generalized to many mathematical spaces. Particularly, Poisson process on a three dimensional sphere surface is useful. Nicely enough, Poisson process on a sphere is equivalent to process in a two dimensional area $ A = [-\pi,\pi] \times [-1,1]$ through the area preserving mapping from $A$ to surface of a sphere
        \begin{equation}
...
Mobile telephone signal propagation in a city is characterized essentially by obstacles, such as buildings and cars, that \htmladdnormallink{attenuates}{https://en.wikipedia.org/wiki/Attenuation}, \htmladdnormallink{reflects}{https://en.wikipedia.org/wiki/Reflection_(physics)}, \htmladdnormallink{refracts}{https://en.wikipedia.org/wiki/Reflection_(physics)} and \htmladdnormallink{diffracts}{https://en.wikipedia.org/wiki/Diffraction} the signal.

Should there be a pure \htmladdnormallink{line-of-sight}{https://en.wikipedia.org/wiki/Line-of-sight_propagation} from the transmitter to the receiver, the receiver will always receive a constant power from the transmitter conditioned that the transmitter stays at a constant distance from the receiver – this can be the case for example if you are transmitting to a near-by high base-station antenna. If there is no line-of-sight component, the signal strength will vary according to \htmladdnormallink{Rayleigh fading}{\ray} as the transmitter and/or the obstacles moves. In the case that there is only a limited line-of-sight element present, the signal strength will follow \htmladdnormallink{Rician fading}{https://en.wikipedia.org/wiki/Rician_fading} statistics. 

I animated couple of GIFs to help us to perceive what is going on.

The first figure demonstrates how a simple sine signal propagates between buildings. You can observe how the multi-path components sums up to a signal that is amplifying in some locations and degenerating in others. You can look for both Rician conditions (lower right) and Rayleigh conditions (lower left) in the image and see how the signal behaves under those conditions. Figure was obtained by solving the \htmladdnormallink{Helmholtz equation}{https://en.wikipedia.org/wiki/Helmholtz_equation} by \htmladdnormallink{finite element method}{https://en.wikipedia.org/wiki/Finite_element_method}.

The second figure shows how aggregate signal power from many mobile transmitters develops in time under Rician fading conditions. The aggregate power can be considered as \htmladdnormallink{interference}{https://en.wikipedia.org/wiki/Electromagnetic_interference} in a receiver. You can see how the high peaks of interference power emerges at random locations. In real life, these kinds of peaks can cause decreased data-rates. The figure was obtained by simulating a random walk of points in a realization of the \htmladdnormallink{Poisson point process}{https://en.wikipedia.org/wiki/Poisson_point_process} on a plane.
...
will sometimes get buried completely under the noise – these events are referred to as \textbf{deep fades}.
...
Everyone have some experience how radio turns into static when one is driving into a tunnel. It just does not work (unless there is a special \htmladdnormallink{radiating cable}{https://en.wikipedia.org/wiki/Leaky_feeder} installed inside the tunnel).
...
\item \htmladdnormallink{Interface conditions for electromagnetic fields (Wikipedia article)}{https://en.wikipedia.org/wiki/Interface_conditions_for_electromagnetic_fields}
  ...
  \subsection{September – A pile of sand meeting the Poisson point process}

  I thought that it would be nice to write about some purely mathematical subject today, so I decided to briefly discuss the Poisson process, and its properties under a transformation.

  It is a well known fact that some point processes converges to Poisson point process after sufficient transformations. This is of course why the Poisson p.p. can appears everywhere in the nature; for example, grain of sands in an heap of grains – made by a child at a beach on a sunny day – will eventually spread by wind (or water) around the beach. The grains will be randomly displaced here and there forming something approximating a realization of a Poisson p.p., even though the original heap was more ordered.

  Let's play the game with a heap (or, in fact pile) of sand spreading to a flat area by random translations of the grains. We don't exactly derive any limiting results, because we make the Poisson assumption all way. However, we will show that falling a pile of sand will result in Poisson distributed grains in the ground.


  Suppose a unit circle
  $$
  U = B(0,1)
  $$
  
  with a ``heap'' of sand at origo. Lets make an natural assumption that the amount of grains in the heap is  \htmladdnormallink{Poisson distributed}{https://en.wikipedia.org/wiki/Poisson_distribution}. 

  Then assume that after a while, wind moves the individual sand grain from the to a uniformly disributed angle and random randomply length between $(-\infty, \infty)$. In other words, the probability that a sand grain is moved from the origo to a location $Y_j$ inside a cirle $B(r_1,r_2)$ is $2\int_{r_1}^{r^2} \frac{1}{\sqrt{2 \pi}}e^{y^2} dy.$ 
  
  After a random translation of all points the Laplace Transform is then
   
  \begin{align*}
    \mathcal{L}(f) &= \mathbb{E} \exp \left[-\sum_i f(Y_i) \right] = \mathbb{E}  \prod_i \exp \left[  -f(Y_i)   \right]  \\
    &= \mathbb{E}_n \left[  (2 \pi)^{-n/2}  \int_0^{\infty} \dots \int_0^{\infty}  \prod_{i = 0}^n \exp \left[  -f(y_i)  \right] \frac{1}{\sqrt{2 \pi}}e^{y_i^2} dy_1 \dots d y_n \right] \\
    &= \mathbb{E}_n \left[(2 \pi)^{-n/2}  \left( \int_{0}^{\infty}  \exp \left[  -f(y) - y^2   \right] dy \right)^n\right] \\
    &= \mathbb{E}_n \exp \left[ \sum_{i= 0}^n \log \left(   \int_{0}^{\infty}  \exp \left[  -f(y)  \right] \frac{e^{-y^2}}{(2 \pi)^{-1/2}} dy \right) \right] \\
  \end{align*}

  Evaluating now the Laplace functional with $- \log \left(   \int_{0}^{\infty}  \exp \left[  -f(y)  \right] \frac{e^{-y^2}}{(2 \pi)^{1/2}} dy \right) \right] $
    \begin{align*}
      \mathcal{L} &= \exp \left[-\int_{\mathbb{R}^2} 1 -  e^{- \log \left(   \int_{0}^{\infty}  \exp \left[  -f(y) + y^2 \right] (2 \pi)^{1 - 1/(2x)} dy \right)} \Lambda(dx)   \right]\\
      &= \exp \left[-\int_{\mathbb{R}^2} \left(1 -    \int_{0}^{\infty}  \exp \left[  -f(y) - y^2 \right] (2 \pi)^{1 - 1/(2 x)} dy \right)  \Lambda(dx)  \right] \\
      &= \exp \left[-\int_{\mathbb{R}^2} \left( 1 -    \int_{0}^{\infty}  \exp \left[  -f(y)\right] \frac{e^{-y^2}}{ (2 \pi)^{1 - 1/(2 x)}} dy \right) \Lambda(dx)  \right] \\
      &=  \exp \left[-N -  \left(  -  \int_{0}^{\infty}  \exp \left[  -f(y)  \right] \exp{y^2} (2 \pi)^{1 - 1/(2 N)} dy \right)   \right] \\
      &=  \exp \left[  -  \int_{0}^{\infty}  1- \exp \left[  -f(y)  \right] \exp{y^2} (2 \pi)^{1 - 1/(2 N)} dy    \right] 
    \end{align*}
