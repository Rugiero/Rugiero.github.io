...
\item \htmladdnormallink{Interface conditions for electromagnetic fields (Wikipedia article)}{https://en.wikipedia.org/wiki/Interface_conditions_for_electromagnetic_fields}
  ...
  I thought that it would be nice to write about some purely mathematical subject today, so I decided to briefly discuss about the Poisson process, and its properties under a transformation.
  ...
  Then assume that after a while, wind moves the individual sand grain from the to a uniformly disributed angle and random randomply length between $(-\infty, \infty)$. In other words, the probability that a sand grain is moved to a location inside a cirle $B(r_1,r)$ is $\int \frac{1}{\sqrt{2 \pi}}e^{x^2} dx$

  After a random translation of all points the Laplace Transform is then

  \begin{align*}
    \mathcal{L}(f) &= \mathbb{E} \exp \left[-\sum_i f(Y_i) \right] = \mathbb{E} 2 \pi \prod \int_{-\infty}^{\infty}   \left[e^{-f(y_i)} \right] \frac{1}{\sqrt{2 \pi}}e^{y_i^2} dy_i \\&
    \end{align*}
