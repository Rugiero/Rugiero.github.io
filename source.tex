...
%%         \item Ilari Angervuori, Wichman Risto, Niloofar Okati, Taneli Riihonen, ``On routing protocols in inter-satellite communications,'' Proceedings of XXXV Finnish URSI Convention on Radio Science, 2019
        ...
        \end{figure}
                 ...
                 \begin{enumerate}
                 \item[5.]{Under fairly general settings (for example, assuming a Gaussian waveform for each interferer), the interference waveform at the LEO satellite is approximately 
                   $$I=I(t)=\sqrt{P(t)}X(t), $$
                 ...
                 \item[6.]{The normalized interference power (\textit{i.e.}, variance of $I$) is approximately \htmladdnormallink{gamma distributed}{https://en.wikipedia.org/wiki/Gamma_distribution} with the mean $\mathbb{E}(P)=1$ and the variance $\text{var}(P)=1/2$ (see \htmladdnormallink{[4]}{https://arxiv.org/pdf/2607.11602}, Eqs. (18), (19) and (29)).}
...
If unfixed, $k$, $D$, and the first- and second-order statistics of $P$ are \htmladdnormallink{hyperparameters}{https://en.wikipedia.org/wiki/Hyperparameter_(machine_learning)} learned from the data in the machine learning process. Then the gamma distribution model $6$, combined with the correlation model $7$, forms the hypothesis class within which we search for the interference power $P$. In the correlation model, too large or too small $D$ yields \htmladdnormallink{overfitting}{https://en.wikipedia.org/wiki/Overfitting} and underfitting, respectively. 
...
In Figures 1 (a) and (b), we estimate $P(t)$ from a sampled ($0.1$ kHz) realization of the interference waveform $I(t)$ by using GPR and non-\htmladdnormallink{causal}{https://en.wikipedia.org/wiki/Causal_filter} MA. For simplicity, we use the model parameter values derived from the theoretical SG model (not treating them as hyperparameters). We use the samples from $|I(t)|^2$ to estimate the interference power $P(t)$. Each sample $z_i$ is interpreted as a noisy measurement of $P(t)$: $z_i= P(t_i) + \mathcal{N}(0,\sigma^2)$, where $\sigma^2=3$ is set to the expected variance of $|I(t)|^2$. The MA window size is empirically optimized, and apart from the slight overfitting, the MA captures $P(t)$ reasonably well. The GPR prediction clearly captures the smoothness of $P(t)$ better. Furthermore, the GPR prediction captures $P(t)$ from $t=0$ onward, which is impossible for the non-causal, centralized MA, which starts at $t\approx 0.5$ s. The \htmladdnormallink{forecast}{https://en.wikipedia.org/wiki/Predictive_analytics} region in the figure corresponds to a causal estimation of the future signal based on the past values. Both predictions improve as the sampling frequency is increased.


This was a simple hands-on example of how SG is utilized in ML satellite communication models.
