...
The following GNU Octave code produces plots of bandwidth limited analog Gaussian and  $\alpha$-stable noises. The function \htmladdnormallink{digitaltoanalog}{https://ilari.angervuori.fi/node24.html} is given my May's blog entry. Also, remember to import to statistical package to Octave.
...
\end{equation}
thus the identity $\mathbb{E}[(S_1 + S_2)^2] = \mathbb{E}[S_1^2] + \mathbb{E}[S_2^2] $ holds if and only if the \htmladdnormallink{inner-product}{https://en.wikipedia.org/wiki/Inner_product_space#Random_variables}, or the  \htmladdnormallink{cross-correlation}{https://en.wikipedia.org/wiki/Cross-correlation}, $\mathbb{E}[S_1 S_2] = 0$ -- that is -- if the random variables (or signals) $S_1$ and $S_2$ are not correlated. Clearly, this is not the case with our initial signals, as then the cross-correlation (with time shift $\tau =0$) is given by
...
Let us study the thickness of the tail distributions of three related distributions. The asymptotic decay rate measures the thickness of a random variable's $X$ tail distribution. It is defined by
\begin{equation}
...
\begin{align}
  ...
  &= \lim_{x\rightarrow \infty} -\frac{\log (\Gamma(k,x/\theta))}{x} = \lim_{x\rightarrow \infty} -\frac{\log\left(x^{k-1}e^{-x/\theta}\right) }{x}\label{line:22}\\
  &= \lim_{x\rightarrow \infty} -\frac{\log \left(e^{-x/\theta}\right)}{x} = 1/\theta. 
\end{align}
In line $(7)$, we used the asymptotic behavior of the gamma distribution
$$
...
\begin{align}
  \rho_{\text{Normal}} &= \lim_{x\rightarrow \infty} -\frac{\log \mathbb{P}(X>x)}{x} = -\lim_{x\rightarrow \infty} \frac{\log \text{erfc}\left(\frac{x-\mu}{\sigma \sqrt{2}}\right)}{x} \geq -\lim_{x\rightarrow \infty} \frac{\log\left( \sqrt{\frac{e}{2\pi}}e^{-2\left(\frac{x-\mu}{\sigma \sqrt{2}}\right)^2} \right)}{x} \\
  &\geq -\lim_{x\rightarrow \infty} \frac{\log\left( e^{-2\left(\frac{x-\mu}{\sigma \sqrt{2}}\right)^2} \right)}{x} = -\lim_{x\rightarrow \infty} \frac{-2\left(\frac{x-\mu}{\sigma \sqrt{2}}\right)^2} }{x} = \frac{1}{\theta^2 2}\lim_{x\rightarrow \infty} \frac{x^2-2 x \mu + \mu^2}{ x}  = \infty,
\end{align}
where in line $(9)$, we used the inequality $$\text{erfc}(x) \geq \sqrt{\frac{{2 e}}{\pi}} \frac{\sqrt{\beta-1}}{\beta} e^{-\beta x^2},$$ that holds for $x \geq 0$ and $\beta >1$ (we used $\beta = 2$).


The asymptotic decay rates for the exponential distribution, gamma distribution, and normal distribution are given in $(6)$, $(8)$, and $(10)$, respectively. We see that the decay rate $\rho$ of the normal distribution is always maximal, i.e. $\rho_{\text{Normal}} = \infty$, whereas the decay rates of the exponential distribution and the gamma distribution depend on the scale parameter $\theta$.
