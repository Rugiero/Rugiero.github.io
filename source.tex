...
I. Angervuori, M. Haenggi and R. Wichman, ``Meta Distribution of the SIR in a Narrow-Beam LEO Uplink,'' in IEEE Transactions on Communications, 2025, doi: 10.1109/TCOMM.2025.3547734.
        ...
        \end{itemize}        
                 ...
                 Let me formulate the working definitions of the ML and SG. We focus on the signal processing of wireless networks, particularly interference modeling and its statistical inference.
                 \begin{itemize}
                 \item{SG encompasses \textbf{random models} of wireless networks, from which many statistical properties, such as interference and its temporal and spatial correlations, can be inquired. The expressions are usually presented in mathematical forms; at best, tractable and closed-form formulas are available.} 
                 \item{ML encompasses learning algorithms, from which many statistical properties can be numerically presented according to the past \textbf{raw data}, such as interference prediction, including its temporal and spatial correlation properties. The results are usually numerical.}
                 \end{itemize}

                 
                 The landscape of the ML algorithms is vast; however, they are often based on \htmladdnormallink{Gaussian process regression}{https://en.wikipedia.org/wiki/Kriging} (GPR). The GPR is a signal prediction method that is based on estimating the signal from a \htmladdnormallink{correlation}{https://en.wikipedia.org/wiki/Correlation} function, or more specifically, an \htmladdnormallink{autocovariance}{https://en.wikipedia.org/wiki/Autocorrelation} function (also referred to as a kernel). The autocovariance function can be arbitrary: for example, one may numerically estimate the autocovariance from an empirical data set. However, this approach can lead to uncontrollable numerical problems, especially if the \htmladdnormallink{training data}{https://en.wikipedia.org/wiki/Training,_validation,_and_test_data_sets} is not encompassing enough. Hence, it is useful if the predictions have a prior correlation function estimate based on a theoretical model, which will guide us in the right direction for inferring the empirical data at hand.

                 In the following, we demonstrate a simple example of estimating the power of a non-stationary Gaussian interference signal at a \htmladdnormallink{low Earth orbit}{https://en.wikipedia.org/wiki/Low_Earth_orbit} base station (LEO BS). The prior autocovariance function is derived from a theoretical estimate based on SG. Based on this autocovariance, the power estimation is inferred from empirical data using GPR. Furthermore, the estimation is compared to a \htmladdnormallink{moving average}{https://en.wikipedia.org/wiki/Moving_average} (MA) estimation (which is not ML).

                 The theoretical settings are:
                 \begin{enumerate}
                 \item{The interferers (natural sources or transmitters) of bandwidth $1$ kHz are distributed according to the \htmladdnormallink{Poisson point process}{https://en.wikipedia.org/wiki/Poisson_point_process} (PPP) on the Earth surface, causing interference at the LEO BS.} 
                 ...
                 \end{enumerate}
                 As the LEO BS moves, the magnitude of the aggregate interference varies due to the randomly distributed interferers. The key insight acquired from the SG analysis is as follows. (The details can be found in \htmladdnormallink{my thesis, Sections 3.3 and 3.4}{dissertation.pdf}.)

                 \begin{enumerate}
                 \item{Under fairly general settings, the interference waveform at the LEO BS is approximately \begin{equation}
                     I(t)=\sqrt{P(t)}X(t), \tag{1}\end{equation} where $X(t)$ is a Gaussian noise waveform modulated by the power $P(t)$.}
                 \item{The normalized interference power $P=P(t)$ at the LEO BS is approximately \htmladdnormallink{gamma distributed}{https://en.wikipedia.org/wiki/Gamma_distribution} with the mean $\mathbb{E}(P)=1$ and the variance $\text{var}(P)=1/2$.}
                 \item{The autocovariance of $P(t)$ at lag $\tau$ has the Gaussian form \begin{equation}K_{P}(\tau)=k \exp\left\{D\tau^2 \log(2)\right\}=1/2 \exp\left\{v_{\text{sat}}^2\tau^2/(h^2 \varphi_{\text{RX}}^2) \log(2)\right\}. \tag{2}\end{equation}}
                 \end{enumerate}
                 This is the prior model we will utilize in the GPR. The described signal power $P(t)$ with the \htmladdnormallink{Gaussian autocovariance}{https://en.wikipedia.org/wiki/Radial_basis_function_kernel} is the hypothesis class of the set of functions within which we search our estimation from, and the waveform $I(t)$ is treated as a non-stationary \htmladdnormallink{Gaussian process}{https://en.wikipedia.org/wiki/Gaussian_process}. In practice, we will use the samples from $|I(t)|^2$ to estimate the interference power $P(t)$. Each such sample $z_i$ is interpreted as noisy measurements of $P(t)$: $z_i= P(t_i) + \mathcal{N}(0,\sigma^2)$, where $\sigma^2=3$ is set to correspond to the expected variance of $|I(t)|^2$. 

                   Note that, given an antenna pattern width, the constant $D=v_{\text{sat}}^2\tau^2/(h^2 \varphi_{\text{RX}}^2)$ in (2) is directly defined by the altitude $h$ of the satellite: the \htmladdnormallink{orbital speed}{https://en.wikipedia.org/wiki/Orbital_speed} $v_{\text{sat}}$ follows single-handedly from the orbital altitude. On the contrary, the scaling parameter $k=1/2$ would generally have a dependency on the density of the interferers (in our case, the received power being normalized). However, it can be empirically verified that within a fairly general density region, the assertion of $D$ is more crucial for the GPR estimation (especially in the non-causal prediction): too large and too small $D$ yield \htmladdnormallink{overfitting}{https://en.wikipedia.org/wiki/Overfitting} and underfitting the data, respectively. In a refined version of the GPR, $k$ and $D$ could also be treated as \htmladdnormallink{hyperparameters}{https://en.wikipedia.org/wiki/Hyperparameter_(machine_learning)}, which are learned from the data, but we treat them as fixed.
