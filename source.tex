...
Noise is often modelled as \htmladdnormallink{Gaussian}{https://en.wikipedia.org/wiki/Gaussian_noise}. However, it can be an unrealistic model in certain scenarios if the ``rare events'' are ``less rare'' than in the Gaussian distribution. Particularly, this is the case with \htmladdnormallink{fat tailed distributions}{https://en.wikipedia.org/wiki/Fat-tailed_distribution}, that have a significant mass in the tail distributions. Other impulsive noise distributions (that are not strictly speaking fat-tailed or heavy-tailed) are the Middleton distributions, which are derived for the envelope of the noise from a non-Gaussian source.
