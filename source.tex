...
\item \htmladdnormallink{Interface conditions for electromagnetic fields (Wikipedia article)}{https://en.wikipedia.org/wiki/Interface_conditions_for_electromagnetic_fields}
  ...
  Consider a pile of sand at origo. Let's assume that the pile is small enough so that it is concerned in a single point, i.e. the origo. Let's make a natural assumption that the amount of grains in the pile is  \htmladdnormallink{Poisson distributed}{https://en.wikipedia.org/wiki/Poisson_distribution}. 
  ...
  In fact, the result above applies to any translations as long as the translation is independently applied to every point (or grain); the Poisson point process remains Poisson in a transformation. Yes, we had to make the Poisson assumption also for the pile, so that the result above could be derived. But indeed, we can show that certain non-Poisson processes approach Poisson p.p. when the points are translated. However, I leave this is out of the scope of this blog entry for now, and take the remark made here as a simple (possibly  a bit weird) example of the universality of the Poisson point process.
