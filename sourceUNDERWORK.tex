...
Here is a representation of the \htmladdnormallink{hypergeometric function}{https://en.wikipedia.org/wiki/Generalized_hypergeometric_function} $_3F_2(1,1,b;2,2;\cdot)$ in terms of the \htmladdnormallink{polylogarithm}{https://en.wikipedia.org/wiki/Polylogarithm}: For  $|x|<1$ and $b\in\mathbb{N}$, the \textit{hypergeometric series} representation is given by
                 ...
                 Finally, time for the blog post of the year $2025$. As the reader may presume, I have been very busy with the PhD thesis and its articles in the past years. Now being applying to jobs, I have encountered the obvious trend that artificial intelligence (AI) and machinge learning (ML) (one might argue if the terms are interchangable, however, I prefer the latter term as an umbrella) are the hot topic of the day. This is why I feel required to address how stochastic geometry (SG), which is sometimes seen even as a rival to ML, has and will be utilized in the ML.

                 Sometimes, SG and ML has been even seen as rivals: if so, ML community can declare the victory at least of the battle, maybe for the war in the purely economical performance meters: SG transformed from an academic exercice into the industry only in mariginal sense, however, as of today, the ML is encompassed everywhere. However, rivalry is not the only to see, and definitely not the most constructive. Is the (extensively ongoing) study of SG useless? No it is not? Let us address why. 

                 My quick working definitions of the ML and SG. We focus on the signal processing of wireless networks aspects i nthe following, and it might be appropriate to further narrow the framework into interference modeling, which is also embeds into fairly general context in the wireless network study (since the interference is the crucial limiting factor in many networks).
                 \begin{itemize}
                 \item{SG encompasses random \textbf{models} of wireless networks, from which many statistical properties; such as of interference and its temporal and spatial correlations. The expressions are usually presented in mathematical forms; at best in tractable and closed form.} 
                 \item{ML encompasses learning algorithms, from which many statistical properties can be numerically many times presented according to the past \textbf{raw-data}, such as interference prediction including its temporal and spatial correlation properties. The results are usually numerical (or at least has an equivalent intepretation with numericals).}
                 \end{itemize}
